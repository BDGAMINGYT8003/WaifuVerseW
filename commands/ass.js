...
module.exports = {
    name: 'ass',
    description: 'Get a random NSFW ass image or GIF',
    async execute(ctx) {
        ...
        if (remainingCooldown > 0) {
            ...
            return ctx.reply(
                `This command has a rate limit to ensure optimal performance. Please try again after ${seconds} seconds.`,
                { parse_mode: 'Markdown' }
            );
        }
                ...
                try {
                    const requests = Array(CONFIG.CONCURRENT_REQUESTS)
                        ...
                        .map(() => circuitBreaker.execute(() =>
                            api.get('/search', {
                                params: { included_tags: 'ass', is_nsfw: true },
                            })
                        ));
        ...
        try {
            ...
            console.error('Failed to fetch ass content:', error);
            await ctx.reply('*Oops!* I couldn\'t fetch ass content at the moment. Please try again later.', {
                parse_mode: 'Markdown',
            });
        }
    }
