...
module.exports = {
    name: 'maid',
    description: 'Get a random maid image',
    async execute(ctx) {
                ...
                try {
                    const requests = Array(CONFIG.CONCURRENT_REQUESTS)
                        ...
                        .map(() => circuitBreaker.execute(() =>
                            api.get('/search', {
                                params: { included_tags: 'maid', is_nsfw: false },
                            })
                        ));
        ...
        try {
            ...
            await ctx.replyWithPhoto(
                { url: imageUrl },
                {
                    caption: `🧹 Here's a maid image for you!\n\n[Click here if the image doesn't load properly](${imageUrl})`,
                    parse_mode: 'Markdown',
                    disable_notification: true,
                }
            );
        } catch (error) {
            console.error('Failed to fetch maid image:', error);
            await ctx.reply('*Oops!* I couldn\'t fetch a maid image at the moment. Please try again later.', {
                parse_mode: 'Markdown',
            });
        }
    }
