...
module.exports = {
    name: 'ero',
    description: 'Get a random ero image or GIF',
    async execute(ctx) {
                ...
                try {
                    const requests = Array(CONFIG.CONCURRENT_REQUESTS)
                        ...
                        .map(() => circuitBreaker.execute(() =>
                            api.get('/search', {
                                params: { included_tags: 'ero', is_nsfw: true },
                            })
                        ));
        ...
        try {
            ...
            console.error('Failed to fetch ero content:', error);
            await ctx.reply('*Oops!* I couldn\'t fetch ero content at the moment. Please try again later.', {
                parse_mode: 'Markdown',
            });
        }
    }
