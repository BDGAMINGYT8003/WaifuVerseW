...
module.exports = {
    name: 'oppai',
    description: 'Get a random oppai image',
    async execute(ctx) {
                ...
                try {
                    const requests = Array(CONFIG.CONCURRENT_REQUESTS)
                        ...
                        .map(() => circuitBreaker.execute(() =>
                            api.get('/search', {
                                params: { included_tags: 'oppai', is_nsfw: false },
                            })
                        ));
        ...
        try {
            ...
            await ctx.replyWithPhoto(
                { url: imageUrl },
                {
                    caption: `👙 Here's an oppai image for you!\n\n[Click here if the image doesn't load properly](${imageUrl})`,
                    parse_mode: 'Markdown',
                    disable_notification: true,
                }
            );
        } catch (error) {
            console.error('Failed to fetch oppai image:', error);
            await ctx.reply('*Oops!* I couldn\'t fetch an oppai image at the moment. Please try again later.', {
                parse_mode: 'Markdown',
            });
        }
    }
